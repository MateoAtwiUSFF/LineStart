<script lang="ts">
	...
	let selectedDays = $state<number[]>([1, 2, 3, 4, 5]); // Weekdays by default
	...
	async function handleSubmit() {
		...
		try {
			...
			await resourcesStore.create(resourceData);
	...
	>
		...
		>
			...
			<div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
				<h2 id="modal-title" class="text-xl font-bold text-gray-900">Add New Resource</h2>
				<button
					onclick={close}
